...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        type=str,
        default="Whatever",
        help="Name of the species (e.g. grant)"
    )
...
def get_full_path(direc_path, rel_path):
    ...
    # try:
    #     return os.readlink(os.path.join(direc_path, rel_path))
    # except OSError:
    #     return os.path.join(direc_path, rel_path)
...
def main(argv):
    ...
    config_yaml["species"] = args.species
    ...
    if not bams:
        ...
        print(f"Found {len(bams)} bams", file=sys.stderr)
