...
PSMC_DIR="/artemis-internal/casia16/program/psmc"
BCFTOOLS="/isdata/hellergrp/nzg134/programs/bin/bcftools"
VCFUTILS="/artemis-internal/casia16/program/samtools/bcftools/vcfutils.pl"
...
rule gen_bcftools_genome_wide:
    ...
    threads: 2
    shell:
        "{BCFTOOLS} mpileup --threads {threads} --full-BAQ -T {BED} -Q {MIN_BQ}  -q {MIN_MQ} -O u --fasta-ref {FA} --per-sample-mF -a FORMAT/AD,FORMAT/DP {input} | {BCFTOOLS} call -Ob -o {output} --threads {threads} -c"
...
rule filter_bcf:
    ...
    threads: 2
    shell: """
    set +o pipefail;{BCFTOOLS} view --threads {threads} -i 'strlen(REF)==1 & (strlen(ALT)==1 || ALT=".") &  FMT/DP>={params.mindepth} & FMT/DP<={params.maxdepth}' -M 2 -Ou {input} | \
...
rule gen_fq_mindepthx:
    ...
    shell:
        "set +o pipefail;{BCFTOOLS} view  {input} | {VCFUTILS} vcf2fq | gzip > {output}"
...
rule AC_count:
    ...
    shell:
        "set +o pipefail;{BCFTOOLS} query -f '%INFO/AC1\n' {input} | awk '{params.awk}' > {output}"
