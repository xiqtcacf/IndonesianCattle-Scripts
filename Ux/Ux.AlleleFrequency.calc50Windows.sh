...
outputdir="/binf-isilon/hellergrp/wlk579/Bos_project/Josiah/Xi_AF_calculate_test/"
inputtxt="/binf-isilon/hellergrp/wlk579/Bos_project/Josiah/Xi_AF_calculate_test/$batch.phased233_maf05NEWnaming.abcfiltered.filltags.dAFs.txt" #All sites describing AFs
countFilter="0" #Minimum AC
aFilter="0.05"  #Cattle
bFilter="0.5"   #madura  ######Xi changing herer Not Using bfiltering
cFilter="0.9"   #Banteng
windowsfile="/binf-isilon/hellergrp/wlk579/Bos_project/Josiah/madura/bosARS-UCD1.3.50kbwindows.bed"

bedtools=/isdata/hellergrp/wlk579/software/bedtools2/bin/bedtools

#Output variables
filteredSites="$outputdir/`basename $inputtxt .txt`.${countFilter}countFilt_${aFilter}a_${cFilter}c.txt"
outputtxt="$outputdir/`basename $windowsfile .bed`.`basename $inputtxt .txt`.${countFilter}countFilt_${aFilter}a_${cFilter}c.abcSNPcounts.txt"
...
#Filtered sites
#Filter for ≤0.05, ≥0.5/0.8, ≥0.9 Origional  ##########change here only ≤0.05,  ≥0.9
...
tail -n+2 $inputtxt | awk -v countFilter=$countFilter '$6>countFilter'  | awk -v aFilter=$aFilter -v cFilter=$cFilter '$17<=aFilter && $19>=cFilter' OFS="\t" > $filteredSites
cat header.txt $batch.phased233_maf05NEWnaming.abcfiltered.filltags.dAFs.0countFilt_0.05a_0.9c.txt > header.$batch.phased233_maf05NEWnaming.abcfiltered.filltags.dAFs.0countFilt_0.05a_0.9c.txt
